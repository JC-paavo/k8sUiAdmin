<template>
  <div class="resource-list-page">
        ...
        <div class="header-content">
          <div class="header-title-section">
            ...
            <div class="resource-icon">
              <IconStatefulSet :size="48" />
            </div>
            <div class="title-text">
              <h1>StatefulSet 资源管理</h1>
          ...
          <div class="header-stats">
            <div class="stat-card">
              ...
              <div class="stat-label">StatefulSet 总数</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="resource-table-container">
        ...
        >
          <el-table-column prop="metadata.name" label="名称" min-width="200">
            <template #default="scope">
              <span class="clickable-name" @click="viewResource(scope.row)">
                <el-icon class="resource-icon-small"><Database /></el-icon>
          ...
          <el-table-column label="状态" width="120">
            <template #default="scope">
              <el-tag :type="getStatefulSetStatusTagType(scope.row)" size="small">
                {{ getStatefulSetStatus(scope.row) }}
              </el-tag>
            </template>
          </el-table-column>
        ...
        <div v-if="filteredResources.length === 0" class="empty-state">
          ...
          <h3>暂无 StatefulSet</h3>
          <p v-if="searchQuery">未找到匹配 "{{ searchQuery }}" 的 StatefulSet</p>
          <p v-else>该命名空间下暂无 StatefulSet 资源</p>
          <el-button 
            type="primary" 
            @click="showCreateModal = true" 
    ...
    <!-- 创建 StatefulSet 对话框 -->
    <el-dialog title="创建 StatefulSet (YAML)" v-model="showCreateModal" width="900px" :close-on-click-modal="false">
...
spec:
  ...
  template:
    ...
    <!-- 编辑对话框 -->
    <el-dialog title="编辑 StatefulSet (YAML)" v-model="showEditModal" width="900px" :close-on-click-modal="false">
      <div class="yaml-editor-container">
        <el-input
          v-model="editYaml"
          type="textarea"
...
import IconStatefulSet from '@/assets/icons/IconStatefulSet.vue'
...
const getStatefulSetStatus = (row) => {
...
const getStatefulSetStatusTagType = (row) => {
  const ready = row.status.readyReplicas || 0
  const total = row.status.replicas || 0
...
const viewResource = (resource) => {
  ...
  router.push(`/clusters/${id}/statefulsets/${namespace}/${name}`)
}
...
const handleEdit = async (resource) => {
  ...
  try {
    const id = clusterId.value
    const response = await k8sAPI.getStatefulSet(id, resource.metadata.namespace, resource.metadata.name)
    try {
...
const handleUpdate = async () => {
  ...
  try {
    ...
    const id = clusterId.value
    await k8sAPI.updateStatefulSet(id, currentResource.value.metadata.namespace, currentResource.value.metadata.name, updatedResource)
...
const fetchEvents = async () => {
  ...
  try {
    const response = await k8sAPI.getStatefulSetEvents(
      id, 
...
const handleDelete = async (resource) => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除 StatefulSet ${resource.metadata.name} 吗？`,
      '提示',
      {
    ...
    await k8sAPI.deleteStatefulSet(
      id, 
...
const handleCreate = async () => {
  ...
  try {
    const statefulSet = jsYaml.load(createYaml.value)
    
    await k8sAPI.createStatefulSet(id, statefulSet)
...
const fetchResources = async () => {
  ...
  try {
    const response = await k8sAPI.listStatefulSets(id, selectedNamespace.value)
...
onMounted(async () => {
  console.log('StatefulSetList onMounted, route.params:', route.params)

<template>
  <div class="resource-list-page">
    ...
    <div v-else class="page-container">
        ...
        <div class="header-content">
          ...
          <div class="header-stats">
            <div class="stat-card">
              ...
              <div class="stat-label">ConfigMap 总数</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="resource-table-container">
        ...
        >
          <el-table-column prop="metadata.name" label="名称" min-width="200">
            <template #default="scope">
              <span class="clickable-name" @click="viewResource(scope.row)">
                <el-icon class="resource-icon-small"><Setting /></el-icon>
                {{ scope.row.metadata.name }}
              </span>
            </template>
          </el-table-column>
          <el-table-column prop="metadata.namespace" label="命名空间" width="150" />
          <el-table-column label="数据项数" width="120">
            <template #default="scope">
              <el-tag type="info" size="small">{{ Object.keys(scope.row.data || {}).length }}</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="180" fixed="right" :cell-style="{ textAlign: 'center' }">
            <template #default="scope">
        ...
        <div v-if="filteredResources.length === 0" class="empty-state">
          ...
          <h3>暂无 ConfigMap</h3>
          <p v-if="searchQuery">未找到匹配 "{{ searchQuery }}" 的 ConfigMap</p>
          <p v-else>该命名空间下暂无 ConfigMap 资源</p>
          <el-button 
            type="primary" 
            @click="showCreateModal = true" 
    ...
    <el-dialog title="创建 ConfigMap (YAML)" v-model="showCreateModal" width="900px" :close-on-click-modal="false">
...
data:
  ...
  database_uri: postgresql://localhost:5432/mydb"
    ...
    <el-dialog title="编辑 ConfigMap (YAML)" v-model="showEditModal" width="900px" :close-on-click-modal="false">
      <div class="yaml-editor-container">
        <el-input
          v-model="editYaml"
          type="textarea"
...
const viewResource = (resource) => {
  ...
  router.push(`/clusters/${id}/configmaps/${namespace}/${name}`)
}
...
const handleEdit = async (resource) => {
  ...
  try {
    const id = clusterId.value
    const response = await k8sAPI.getConfigMap(id, resource.metadata.namespace, resource.metadata.name)
    try {
...
const handleUpdate = async () => {
  ...
  try {
    ...
    const id = clusterId.value
    await k8sAPI.updateConfigMap(id, currentResource.value.metadata.namespace, currentResource.value.metadata.name, updatedResource)
...
const handleDelete = async (resource) => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除 ConfigMap ${resource.metadata.name} 吗？`,
      '提示',
      {
...
const handleCreate = async () => {
  ...
  try {
    const configmap = jsYaml.load(createYaml.value)
    
    await k8sAPI.createConfigMap(id, configmap)
...
const fetchResources = async () => {
  ...
  try {
    const response = await k8sAPI.listConfigMaps(id, selectedNamespace.value)
...
onMounted(async () => {
  console.log('ConfigMapList onMounted, route.params:', route.params)

<template>
  <div class="resource-list-page">
    ...
    <div v-else class="page-container">
        ...
        <div class="header-content">
          ...
          <div class="header-stats">
            <div class="stat-card">
              ...
              <div class="stat-label">Ingress 总数</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="resource-table-container">
          ...
          <el-table-column prop="metadata.name" label="名称" min-width="200">
            <template #default="scope">
              <span class="clickable-name" @click="viewResource(scope.row)">
                <el-icon class="resource-icon-small"><Globe /></el-icon>
                {{ scope.row.metadata.name }}
              </span>
            </template>
          </el-table-column>
          <el-table-column prop="metadata.namespace" label="命名空间" width="150" />
          <el-table-column label="主机" min-width="250">
        ...
        <div v-if="filteredResources.length === 0" class="empty-state">
          ...
          <h3>暂无 Ingress</h3>
          <p v-if="searchQuery">未找到匹配 "{{ searchQuery }}" 的 Ingress</p>
          <p v-else>该命名空间下暂无 Ingress 资源</p>
          <el-button 
            type="primary" 
            @click="showCreateModal = true" 
    ...
    <el-dialog title="创建 Ingress (YAML)" v-model="showCreateModal" width="900px" :close-on-click-modal="false">
...
spec:
  ...
  - host: myapp.example.com
    ...
    <el-dialog title="编辑 Ingress (YAML)" v-model="showEditModal" width="900px" :close-on-click-modal="false">
      <div class="yaml-editor-container">
        <el-input
          v-model="editYaml"
          type="textarea"
...
const viewResource = (resource) => {
  ...
  router.push(`/clusters/${id}/ingresses/${namespace}/${name}`)
}
...
const handleEdit = async (resource) => {
  ...
  try {
    const id = clusterId.value
    const response = await k8sAPI.getIngress(id, resource.metadata.namespace, resource.metadata.name)
    try {
...
const handleUpdate = async () => {
  ...
  try {
    ...
    const id = clusterId.value
    await k8sAPI.updateIngress(id, currentResource.value.metadata.namespace, currentResource.value.metadata.name, updatedResource)
...
const handleDelete = async (resource) => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除 Ingress ${resource.metadata.name} 吗？`,
      '提示',
      {
...
const handleCreate = async () => {
  ...
  try {
    const ingress = jsYaml.load(createYaml.value)
    
    await k8sAPI.createIngress(id, ingress)
...
const fetchResources = async () => {
  ...
  try {
    const response = await k8sAPI.listIngresses(id, selectedNamespace.value)
...
onMounted(async () => {
  console.log('IngressList onMounted, route.params:', route.params)

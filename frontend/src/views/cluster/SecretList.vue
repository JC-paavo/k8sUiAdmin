<template>
  <div class="resource-list-page">
    ...
    <div v-else class="page-container">
        ...
        <div class="header-content">
          ...
          <div class="header-stats">
            <div class="stat-card">
              ...
              <div class="stat-label">Secret 总数</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="resource-table-container">
        ...
        <div v-if="filteredResources.length === 0" class="empty-state">
          ...
          <h3>暂无 Secret</h3>
          <p v-if="searchQuery">未找到匹配 "{{ searchQuery }}" 的 Secret</p>
          <p v-else>该命名空间下暂无 Secret 资源</p>
          <el-button 
            type="primary" 
            @click="showCreateModal = true" 
    ...
    <el-dialog title="创建 Secret (YAML)" v-model="showCreateModal" width="900px" :close-on-click-modal="false">
...
data:
  password: dGVzdA=="
    ...
    <el-dialog title="编辑 Secret (YAML)" v-model="showEditModal" width="900px" :close-on-click-modal="false">
      <div class="yaml-editor-container">
        <el-input
          v-model="editYaml"
          type="textarea"
...
const viewResource = (resource) => {
  ...
  router.push(`/clusters/${id}/secrets/${namespace}/${name}`)
}
...
const handleEdit = async (resource) => {
  ...
  try {
    const id = clusterId.value
    const response = await k8sAPI.getSecret(id, resource.metadata.namespace, resource.metadata.name)
    try {
...
const handleUpdate = async () => {
  ...
  try {
    ...
    const id = clusterId.value
    await k8sAPI.updateSecret(id, currentResource.value.metadata.namespace, currentResource.value.metadata.name, updatedResource)
...
const handleDelete = async (resource) => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除 Secret ${resource.metadata.name} 吗？`,
      '提示',
      {
...
const handleCreate = async () => {
  ...
  try {
    const secret = jsYaml.load(createYaml.value)
    
    await k8sAPI.createSecret(id, secret)
...
const fetchResources = async () => {
  ...
  try {
    const response = await k8sAPI.listSecrets(id, selectedNamespace.value)
...
onMounted(async () => {
  console.log('SecretList onMounted, route.params:', route.params)
...
.create-btn:hover {
  box-shadow: 0 6px 16px rgba(245, 175, 25, 0.4);
